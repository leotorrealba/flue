...
- **Workflow and agent modules now reserve only Flue-owned exports.** Workflow modules must directly export `export async function run(...)` and may export `export const channels = [http(), websocket()]`; they can initialize locally declared `createAgent(...)` values when needed but do not default-export a primary agent. Agent modules must default-export `createAgent(...)`, and external-channel agent modules additionally export `channels` and `receive(...)`. Other local exports are allowed for schemas, helper functions, and TypeScript-only payload/result types, but Flue-owned exports still need the direct forms so generated Node and Cloudflare entries stay deterministic.
